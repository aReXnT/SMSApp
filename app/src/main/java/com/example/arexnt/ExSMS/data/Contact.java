package com.example.arexnt.ExSMS.data;
...
import com.android.mms.util_alt.SqliteWrapper;
import com.example.arexnt.ExSMS.LogTag;
import com.example.arexnt.ExSMS.R;
import com.example.arexnt.ExSMS.transaction.SmsHelper;
...
public class Contact {
    ...
    public static String formatNameAndNumber(String name, String number, String numberE164) {
        ...
        if (!SmsHelper.isEmailAddress(number)) {
            ...
            formattedNumber = PhoneNumberUtils.formatNumber(number, numberE164, country.getCountryIso());
        }
    ...
    public synchronized void setNumber(String number) {
        if (!SmsHelper.isEmailAddress(number)) {
            Country country = new Country(Locale.getDefault().getCountry(), Country.COUNTRY_SOURCE_LOCALE);
            mNumber = PhoneNumberUtils.formatNumber(number, mNumberE164, country.getCountryIso());
        } else {
            mNumber = number;
        }
    ...
    private static class ContactsCache {
        ...
        private boolean contactChanged(Contact orig, Contact newContactData) {
            ...
            if (!oldName.equals(newName)) {
                if (Log.isLoggable(LogTag.CONTACT, Log.DEBUG)) {
                    log(String.format("name changed: %s -> %s", oldName, newName));
                }
                return true;
            }
            ...
            if (!oldLabel.equals(newLabel)) {
                if (Log.isLoggable(LogTag.CONTACT, Log.DEBUG)) {
                    log(String.format("label changed: %s -> %s", oldLabel, newLabel));
                }
                return true;
            }
